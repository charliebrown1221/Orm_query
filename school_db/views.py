...
def problem_one(request):
  higher_gpas= Student.objects.filter(gpa__gt=3.0).order_by( '-gpa' )
  for higher_gpa in higher_gpas:
   print(f'Full name: {higher_gpa.first_name} {higher_gpa.last_name}  GPA: {higher_gpa.gpa}')
...
def problem_two(request):
 hiredprofs = Instructor.objects.filter(hire_date__year__lt=2010) 
 for hiredprof in hiredprofs:
   print ( f' Full Name: {hiredprof.first_name} {hiredprof.last_name} Hired Date: {hiredprof.hire_date}')
...
def problem_three(request):
 prof= Instructor.objects.get(id=2)
 courses = Course.objects.filter(instructor_id = 2)
 print(prof.first_name,prof.last_name)
